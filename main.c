...
char *extract_between(const char *str, const char *p1, const char *p2)
{
  ...
  if(i1 != NULL)
  {
    const uint32_t pl1 = strlen(p1);
    char *i2 = strstr(i1 + pl1, p2);
    if(p2 != NULL)
    {
     // Found both markers, extract text. 
     uint32_t mlen = i2 - (i1 + pl1);
...
uint8_t Cmd_Buffer[CMD_BUFF_SZ];
uint8_t cmdBias = 0;
...
volatile uint8_t ADCBuffer_2[16];

void ADC_DMA_init_2(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
	ADC_InitTypeDef ADC_InitStructure;
	DMA_InitTypeDef DMA_InitStructure;
  ...
  RCC_APB2PeriphClockCmd(	RCC_APB2Periph_ADC1 | RCC_APB2Periph_GPIOA, ENABLE);
	...
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)ADCBuffer_2;
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;
	DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&ADC1->DR;
...
extern uint8_t i2c1_ram[I2C1_RAM_SIZE+1];
int main(void)
{
	for(int i = 0; i < I2C1_RAM_SIZE; i++)
	{
		i2c1_ram[i] = i;
	}
...
int main(void)
{
	...
	memset(Cmd_Buffer, 0, CMD_BUFF_SZ);
 ...
 //   usart_init(); //USART
  ...
  /*  		//sprintf(buffer, "\r\n1=%d : 2=%d : 3=%d : 4=%d\r\n", ADCBuffer[0], ADCBuffer[1], ADCBuffer[2], ADCBuffer[3]);
	...
	if (bDeviceState == CONFIGURED)
    {
		...
		if (bDeviceState == CONFIGURED)
		{
		  CDC_Receive_DATA();// Check to see if we have data yet
		  ...
		  if (Receive_length  != 0)
		  {
			 ...
			 /* if (Receive_Buffer[0]=='1') 
			  ...
			  if((cmdBias + Receive_length) >= CMD_BUFF_SZ) //Ovrflow check
			  {
				  	cmdBias = 0; //reset buffer to start
					memset(Cmd_Buffer, 0, CMD_BUFF_SZ);	
			  }
			  else
			  {
				  memcpy(Cmd_Buffer + cmdBias, (void*)Receive_Buffer, Receive_length);
				  cmdBias += Receive_length;
				  Receive_length = 0;
			  }
		  }
		}
		...
		{			
			...
			//Just debug echo
			sprintf(buffer, "\r\n %s", Cmd_Buffer);
			CDC_Send_DATA ((uint8_t*)buffer, cmdBias+2);
			
			char *parsePtr = NULL;
			char *tmpStr = NULL;
			
			parsePtr = strstr(Cmd_Buffer, "i2cset");
			if(NULL != parsePtr)
			{
				tmpStr = strtok(parsePtr, " "); //Skip i2cset
				
				tmpStr = strtok(NULL, " ");
				i2cDevAdr = (uint8_t)strtol(tmpStr, NULL, 0);
				
				tmpStr = strtok(NULL, " ");
				i2cDevReg = (uint8_t)strtol(tmpStr, NULL, 0);
				
				tmpStr = strtok(NULL, " ");
				i2cDevVal = (uint8_t)strtol(tmpStr, NULL, 0);
				
				sprintf(buffer, "i2cDevAdr =  %d, i2cDevReg =  %d, i2cDevVal =  %d \r\n", i2cDevAdr, i2cDevReg, i2cDevVal);			

			//call i2c funktion
			//...

			CDC_Send_DATA ((uint8_t*)buffer, strlen(buffer));
			...
			{			
				parsePtr = strstr(Cmd_Buffer, "i2cget");
				if(NULL != parsePtr)
				{
					tmpStr = strtok(parsePtr, " "); //Skip i2cget
